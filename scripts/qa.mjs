...
const publicData=['data/app-trip.json','data/app-days-1.json','data/app-days-2.json','data/app-days-3.json','data/app-support.json','data/content-places.json','data/content-food.json','data/day1-citywalk.json'].map(read).join('\n');
...
const sw=read('sw.js');
ok(sw.includes("shanghai-family-trip-v2.7"),'service worker cache version is v2.7');
...
console.log(`\nQA complete: ${days.length} days, ${events.length} core activities, ${extraPlaces.places.length} extended places, ${extraFood.places.length} researched food entries, ${verifiedKeys.length} exact-place remote photo mappings.`);
