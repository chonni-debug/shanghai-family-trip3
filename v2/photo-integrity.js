 ...
 * the Chinese place name below, or when it comes from VERIFIED_PHOTO_LIBRARY
 * with attribution metadata. Reused neighbourhood/fallback photos are hidden
 * rather than presented as if they were the exact destination.
 */
...
function auditPlacePhoto(item){
  ...
  if(item.photoVerified===true&&item.photoCredit?.source){
...
function applyPhotoIntegrity(data){
  if(!data)return data;
  if(typeof applyVerifiedPhotoLibrary==='function')applyVerifiedPhotoLibrary(data);
...
function photoCreditBadge(item){
  const c=item?.photoCredit;
  if(!c?.source)return '';
  const label=state.lang==='zh'?`© ${c.author} · 图片来源`:`© ${c.author} · เครดิตรูป`;
  return `<a class="photo-source-badge" href="${esc(c.source)}" target="_blank" rel="noopener" title="${esc(c.license||'')}">${esc(label)}</a>`;
}
...
eventCard=function(e,di,ei){
  ...
  const photo=hasPhoto?`<div class="event-photo"><img src="${esc(e.image)}" alt="${esc(loc(e.name))}" loading="lazy" referrerpolicy="no-referrer"><span class="photo-verified-badge">✓ ${state.lang==='zh'?'地点照片':'รูปสถานที่'}</span>${photoCreditBadge(e)}</div>`:'';
...
placeCard=function(p){
  ...
  const photo=hasPhoto?`<div class="place-photo verified"><img src="${esc(p.image)}" alt="${esc(loc(p.name))}" loading="lazy" referrerpolicy="no-referrer"><span class="photo-verified-badge">✓</span>${photoCreditBadge(p)}</div>`:`<div class="place-photo placeholder"><span>📍</span><small>${p.photoIntegrity==='unverified'?photoMissingLabel():(state.lang==='zh'?'暂无照片':'ยังไม่มีรูป')}</small></div>`;
